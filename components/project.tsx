...
export default function Project(props: Props){
  ...
  return (
    <div ref={ref} className={`${isVisible ? 'opacity-100': 'opacity-0'} duration-1000 relative w-[95%] md:h-[400px] xl:h-[440px] xl:w-3/4 bg-[#e5e5ee] my-3 md:my-5 rounded-lg shadow-lg md:ml-5 xl:ml-[10%] flex flex-col items-center md:flex-row`}>
     <Link className='relative rounded-lg md:m-5 mt-5  md:mr-0 w-[95%] md:max-w-[min(480px,_60%)] lg:max-w-[min(560px,_60%)] xl:max-w-[min(600px,_60%)] overflow-hidden' href={props.source}>
      <Image 
          className='w-full'
          src={props.staticIMG} 
          alt = 'Project Image'
      ...
      <div className='relative flex flex-col md:w-1/2 3xl:w-full items-center mx-3 md:h-[400px] xl:h-[440px]'>
        ...
        <div className={`w-full flex absolute bottom-4 md:bottom-8 justify-around`}>
          <Link href = {props.source} className='flex items-center xl:text-base text-sm 3xl:text-2xl bg-[#B1B1BD] hover:bg-[#9999a3] active:bg-[#83838f] duration-[400ms] py-2 px-4 3xl:py-4 3xl:px-8 rounded tracking-wide shadow-sm'>Source Code {<HiOutlineExternalLink className='ml-1' />}</Link>
          <Link href = {props.live} className={`flex items-center xl:text-base text-sm 3xl:text-2xl bg-[#B1B1BD] hover:bg-[#9999a3] active:bg-[#83838f] duration-[400ms] py-2 px-4 3xl:py-4 3xl:px-8 rounded tracking-wide shadow-sm ${!props.isLive && 'hidden'}`}>Demo {<HiOutlineExternalLink className='ml-1' />}</Link>
        </div>
      </div>
